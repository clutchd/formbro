...
if (!envUrl) {
  if (process.env.VERCEL) {
    throw new Error(
      "APP_URL is not set. Set NEXT_PUBLIC_APP_URL or BETTER_AUTH_URL for production builds.",
    );
  }
  console.warn(
    "[FormBro] NEXT_PUBLIC_APP_URL is not set, using http://localhost:3000 for local development.",
  );
}
